...
const deepCloneFiles = (files) => JSON.parse(JSON.stringify(files));

const INITIAL_FILES_DATA = [
  { name: 'src', type: 'folder', children: [
    { name: 'main.cpp', type: 'file', language: 'cpp', content: 'int main(){return 0;}' }
  ]}
];
...
import { Folder as FolderIcon, FolderOpen, FileText, FileCode2, FileJson, FileType } from 'lucide-react';
...
export default function WorkspaceRootPage() {
  ...
  useEffect(() => {
    if (!activeFile && files.length > 0) {
      const defaultFile = files[0]?.children?.[0] || files[0];
      setActiveFile(defaultFile);
      setCode(defaultFile?.content || '');
      setTitle(defaultFile?.name || 'Synthi Workspace');
      const pathArr = defaultFile ? [files[0], defaultFile].filter(Boolean) : [];
      setBreadcrumb(pathArr);
    }
  }, [files, activeFile]);
              ...
              {breadcrumb && breadcrumb.length > 0 ? (
                breadcrumb.map((node, idx) => (
                  <span key={`${node.name}-${idx}`} className="flex items-center">
                    {node.type === 'folder' ? <FolderIcon className="w-3.5 h-3.5 mr-1 text-gray-400" /> : <FileText className="w-3.5 h-3.5 mr-1 text-gray-400" />}
                    {node.type === 'folder' ? (
                      <Popover>
